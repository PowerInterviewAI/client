...
Always-on-top is a persisted preference (`alwaysOnTop` in `RuntimeConfig`, default on) applied at the `'floating'` level by `applyAlwaysOnTop()`. Stealth mode pins the window at `'screen-saver'` instead, and `disableStealth()` calls `applyAlwaysOnTop()` on the way out rather than clearing the property - clearing it would silently turn the user's preference off on every stealth toggle and every sign-out. Main owns the value: the renderer seeds from `config:get` and follows `window:always-on-top-changed`, because `Ctrl+Shift+T` can change it without the renderer asking.
